...
#
repo_information() {
    echo "%F{2}${vcs_info_msg_0_%%/.} %F{8}$vcs_info_msg_1_`git_dirty` $vcs_info_msg_2_%f"
}
...
precmd() {
    ...
    print -P "\n$(repo_information) %F{3}$(cmd_exec_time)%f"
    unset cmd_timestamp #Reset cmd exec time.
}

# Define prompts
#
PROMPT="%(?.%F{5}.%F{1})❯%f " # Display a red prompt char on failure
